...
class PengaduanController extends CI_Controller
{
	...
	{
		$data['title'] = 'Laporin! | Tulis Laporan';
	...
	{
		$data['title'] = 'Laporin! | History Laporan';
		$masyarakat = $this->db->get_where('masyarakat', ['username' => $this->session->userdata('username')])->row_array();
		$data['data_pengaduan'] = $this->Pengaduan_m->data_pengaduan_masyarakat_nik($masyarakat['nik'])->result_array();
		...
		$this->load->view('_partials/v_sidebar');
		$this->load->view('_partials/v_topbar');
		$this->load->view('masyarakat/history_pengaduan');
		$this->load->view('_partials/v_footer');
		$this->load->view('_partials/v_js');
	}
	...
	public function pengaduan_detail($id)
	{
		...
		if (!empty($cek_data)) :
			...
			$data['title'] = 'Laporin! | Detail Laporan';
			...
			if ($data['data_pengaduan']) :
				...
				redirect('Masyarakat/PengaduanController/history_pengaduan');
			endif;
	...
	public function edit($id)
	{
		...
		if (!empty($cek_data)) :
			...
			if ($cek_data['status'] == '0') :
				...
				$data['title'] = 'Laporin! | Edit Laporan';
				$data['pengaduan'] = $cek_data;

				$this->form_validation->set_rules('jenis_laporan', 'Jenis Laporan', 'trim|required');
				$this->form_validation->set_rules('judul_laporan', 'Judul Laporan', 'trim|required');
				$this->form_validation->set_rules('isi_laporan', 'Isi Lapora', 'trim|required');
				$this->form_validation->set_rules('lokasi_kejadian', 'Lokasi Kejadian', 'trim|required');
				$this->form_validation->set_rules('foto', 'Foto Pengaduan', 'trim');
